...
df <- read.csv("data/alternative_trails_data.csv") %>%
  mutate(
    ...
    popup_text = case_when(
      text != "" ~ paste0("<divs style='display:block;overflow-y:scroll;max-height: 250px;margin-top:5px;margin-bottom:5px'>", text, "</div>"),
    ),
...
pal <- colorFactor(c(col_green, col_purple, col_blue), 
                   domain = c("Organisations", "Meeting Places", "Other"))
...
map <- leaflet(df) %>% 
  ...
  addLegend("bottomleft",
    colors =c(col_green, col_purple, col_blue),
    labels= c("Meeting Places", "Organisations", "Other"),
